	...
	return (
		<div className="container-fluid p-0 pb-4 m-0">
			...
			<div className="d-flex flex-row mx-auto mb-4" style={{ width: "90%", overflow: "auto" }}>
				{store.planets.map((c, i) => <PlanetCards data = {c} name={store.planets[i].name} climate={store.planets[i].climate} population={store.planets[i].population} img="https://upload.wikimedia.org/wikipedia/en/6/6d/Tatooine_%28fictional_desert_planet%29.jpg" key={i}/>)}				
			</div>
		</div>
	);
};
